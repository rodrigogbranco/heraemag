...
class Parse {
	...
	function This_Page($redir="", $meta="") {
		...
		foreach ($tags as $key => $tag) {
			...
			if ($tag{1} != '/') {
				$this->tot[$elem]++;
				$this->tot['total']++;
				$attr_num = preg_match_all('#([^\s=]+)\s*=\s*(\'[^<\']*\'|"[^<"]*")#', $tag, $list_attr);
				$this->tot['total_attr'] += $attr_num;
			}
	...
	function CSS_Absolute($esta_css) {
		...
		$this->tot['cssstatements'] = count($resulta[0]);
	...
	function Define_Results() {
		...
		// Si hay una DTD (y s�lo una)
		// abre la p�gina del validador W3C y lee el resultado
